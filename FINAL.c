...
void compare_stocks(struct Stock *s1) {
    ...
    double numerator = 5 * sumXY - sumX * sumY;
    double denominator = sqrt((5 * sumX2 - sumX * sumX) * (5 * sumY2 - sumY * sumY));
    ...
    if (r > 0.7)
        printf("Strong positive correlation - These stocks tend to move together\n");
    else if (r < -0.7)
        printf("Strong negative correlation - These stocks tend to move in opposite directions\n");
    else if (fabs(r) < 0.3)
        printf("Very weak or no correlation - These stocks' movements are unrelated\n");
    else
        printf("Moderate correlation - Some relationship exists\n");
